...
var minimum_clamp :Vector3 = Vector3.ONE*-1000
var maximum_clamp :Vector3 = Vector3.ONE*1000
...
func update(error:Vector3, delta:float) -> Vector3:
	...
	# if(_error_integral > self.maximum_clamp or _error_integral < self.minimum_clamp):
	# 	reset()
